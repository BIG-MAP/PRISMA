...
class BaselinePeakFitting:
    ...
    def aux_fit_spectrum(self,label):
        ...
        self.spectra[label]['fitted'] = FitPeaks().fit_peaks(self.spectra[label]['processed'], 
    ...
    def aux_update_download_payload(self, label):
        ...
        try:
            individual_dataframes.append(pd.DataFrame(self.spectra[label]['fitted'].counts, columns=['Peak sum'], index=self.spectra[label]['fitted'].energies))
            individual_dataframes += [pd.DataFrame(array, columns=[peak], index=self.spectra[label]['fitted'].energies) for peak, array in self.spectra[label]['fitted'].profiles.items()]
        except KeyError:
            pass 
